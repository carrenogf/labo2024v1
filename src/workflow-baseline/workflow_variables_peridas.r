...
envg$EXPENV <- list()
envg$EXPENV$exp_num <- "variables-perdidas"
...
DR_drifting_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$meta$script <- "/src/workflow-baseline/DR_variables_perdidas.r"
...
ZZ_final_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$meta$script <- "/src/workflow-baseline/z571_ZZ_final.r"
